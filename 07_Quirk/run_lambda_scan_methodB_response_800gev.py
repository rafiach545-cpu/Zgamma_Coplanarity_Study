...
for lambda_ev in lambda_values:
    ...
    for event_id, phi0 in enumerate(
        ...
        # Save raw truth hits for the 800 GeV, 4 keV benchmark
        if lambda_ev == 4000.0:
            raw_path = ROOT / "07_Quirk" / "quirk_hits_truth_800gev_4kev_250gev.csv"
    ...
    truth_out = (
        ROOT
        / "07_Quirk"
        / f"lambda_scan_truth_800gev_{tag}.csv"
    )

    response_out = (
        ROOT
        / "07_Quirk"
        / f"lambda_scan_response_800gev_{tag}.csv"
    )
...
summary_out = (
    ROOT
    / "07_Quirk"
    / "lambda_scan_methodB_summary_800gev.csv"
)
