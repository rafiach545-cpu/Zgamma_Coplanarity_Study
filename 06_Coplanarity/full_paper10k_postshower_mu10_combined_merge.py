...
HARD_PRE  = "05_Tracking/converted_photon_hits_weighted_cylinders_paper10k_postshower_eta18_detector_response_beamspot.csv"
HARD_POST = "05_Tracking/converted_photon_hits_weighted_cylinders_paper10k_postshower_eta18_detector_response_beamspot.csv"  # NOTE: using PRE-merge file as baseline, per decision
...
hard_pre = pd.read_csv(HARD_PRE)
hard_post = pd.read_csv(HARD_POST)
pu = pd.read_csv(PU)

hard_events = sorted(hard_pre["event"].unique())[:NTEST]
...
for n, ev in enumerate(hard_events, 1):
    ...
    h_pre = hard_pre[hard_pre["event"] == ev].copy()
    h_post = hard_post[hard_post["event"] == ev].copy()

    z0_hard = float(h_pre["beamspot_z0"].iloc[0])

    r_hard = run_plane(h_post, z0_hard)
    ...
    # Correct treatment:
    # PU hits are already stored in absolute detector coordinates.
    # Do not subtract vertex_z or zProd before overlay.
    p_corrected = p[["layer_id","x","y","z","r","phi"]].copy()

    combo = pd.concat(
        [
            h_pre[["layer_id","x","y","z","r","phi"]],
            p_corrected[["layer_id","x","y","z","r","phi"]],
        ],
        ignore_index=True
    )
...
out.to_csv(
    "06_Coplanarity/paper10k_mu10_combined_merge_full.csv",
    index=False
)
