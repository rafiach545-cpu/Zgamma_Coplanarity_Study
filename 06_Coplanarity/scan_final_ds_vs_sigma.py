...
SIGMAS = [0.0, 0.1, 0.30, 0.58, 0.87, 1.16]
...
for sigma in SIGMAS:
    ...
    for event, ev in df.groupby("event"):
        ...
        for _, oi, oj in p8:
            for _, si, sj in p7:
                ...
                for lyr in GROWTH:
                    ...
                    for cand in np.where(layers == lyr)[0]:
        ...
        if best is not None:
            ...
            final_ds_values.append(best[0])
            final_dw_values.append(best[1])

    x = np.array(final_ds_values)

    rows.append({
        "sigma_z_mm": sigma,
        "seed_passing_events": n_seed_pass,
        "median_final_ds_mm": np.median(x),
        "p75_final_ds_mm": np.quantile(x,0.75),
        "p90_final_ds_mm": np.quantile(x,0.90),
        "n_ds_below_0p1": int(np.sum(x < 0.1)),
        "fraction_ds_below_0p1": np.mean(x < 0.1),
    })
...
out.to_csv(
    "06_Coplanarity/final_ds_vs_sigma.csv",
    index=False
)
